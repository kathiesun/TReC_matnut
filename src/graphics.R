...
# Example: plot.ci( c(0,10), narrow.intervals=rbind(c(-1,1), c(8,12)), wide.intervals=rbind(c(-3,4), c(5,15)), names=c("Fred", "Barney"))
{
  ...
  if (!add)
  {
    ...
    if (!is.null(main) && is.character(main) && main[1] != "") {
      try(title(main=main, line=main.line, cex.main = cex.main));
    }
    ...
    if (yaxis)
    { 
      axis(2, at=y.pos, labels=rev(names), las=1, lty=0, hadj=0, line=name.line, cex.axis=cex.main)
    }
  }
  before.data()
  if ("p"==type)
  {
    for (i in 1:nvals)
    {
      ...
      lines(midvals[i], name.lines, pos)
    }
  }
  invisible(rev(y.pos))
}
