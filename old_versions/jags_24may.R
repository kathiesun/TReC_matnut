...
for(i in 1:length(ptypes)){
  myPhen[[ptypes[i]]] <- allSummaries_cov(phenotype=ptypes[i], dataf=orderdat, allparam, encoded, 
                                          tryLam=c(-1, 0, .25, .33, .5, 1, 2, 3), Match=F, checkAnova=T, 
...
testBC <- list()
for(i in 1:length(ptypes)){
  testBC[[ptypes[i]]] <- runLMERModels_cov(phenotype=ptypes[i], dataf=orderdat, 
                                           tryLam=c(-1, 0, .25, .33, .5, 1, 2, 3), normd=T, Match=F, checkAnova=T)
}

#form <- formulaWrapper$parseCovariateString(indvariable)

#saveRDS(myPhen, file.path("./","matnut_outputs/",'allPheno_modelfits.rds'))


testPhen <- readRDS("./matnut_outputs/allPheno_modelfits.rds")
test_pred <- prediction(testPhen$WeightPND21$mcmcObject, "WeightPND21", encoded)
ribbon_plot(test_pred, "WeightPND21", encoded)
...
graphics.off()
pdf(file.path("./","matnut_outputs/","allPheno4_ribbon_25may.pdf"), onefile=TRUE, width = 14, height = 11)
for(i in 1:length(ptypes)){  
  print(temp_rib[[ptypes[i]]]$ribbon4)
...
match.multimp <- function(df, matchon, matchoff, ptypes, idcol="ID", n=1)
{
  multimp <- list()
  comb_multimp <- list()
  for(i in 1:length(ptypes)){
    ...
    for(j in 1:n){
      ...
      covariates <- matnut$parameters[-grep(paste("tau","sig",matchoff, sep="|"), matnut$parameters)]
      ...
      temp_jags <- allSummaries_cov(phenotype = ptypes[i], 
                                    dataf=matched_df, allparam, encoded, Match=T)
      tempDF <- as.data.frame(temp_jags$mcmcObject)
      multimp[[ptypes[i]]][[j]] <- as.mcmc(tempDF[,which(colnames(tempDF) %in% encoded$Level)])
    ...
    boundOb <- do.call(rbind, multimp[[ptypes[i]]])
    multimp[[ptypes[i]]]$imp  <- boundOb
    multimp[[ptypes[i]]]$pred <- prediction(mcmcOb=boundOb, ptypes=ptypes[i], encoded, Match=T)
    multimp[[ptypes[i]]]$plot <- ribbon_plot(mcmcOb=multimp[[ptypes[i]]]$pred, n=n, 
                                             ptypes=ptypes[i], encoded, Match=T)
  }
  
}


multimp <- match.multimp(df=orderdat, matchon="Cage", matchoff="PO", idcol="ID")
