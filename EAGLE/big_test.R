...
for (snp.index in 1:n.loci) {
  ...
  x.full[[non.problematic.counter]]=as.matrix(cbind(env=x,intercept=numeric(num.samples)+1.0))
  x.null[[non.problematic.counter]]=as.matrix(cbind(intercept=numeric(num.samples)+1.0))
  # add eQTL heterozygosity
  if (!any(is.na(heteq))) if ((sd(heteq)>0) & (min(table(heteq))>5)){ 
    x.full[[non.problematic.counter]]=as.matrix(cbind(x.full[[non.problematic.counter]],eqtl=heteq))
    x.null[[non.problematic.counter]]=as.matrix(cbind(x.null[[non.problematic.counter]],eqtl=heteq))
      ...
      if ((length(e1)-max(table(e1)))>=10 & (length(e0)-max(table(e0)))>=10){ 
        if (length(unique(x))>2 | min(table(x,heteq))>=5){ 
          ...
          if (det(t(temp) %*% temp) > 1e-8)
            x.full[[non.problematic.counter]]=as.matrix(temp)
        }
      }
    }
...
s$traceEvery=1
unique(unlist(lapply(x.full, class)))
