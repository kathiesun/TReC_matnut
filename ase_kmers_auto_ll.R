...
#for (r in c(2:4,6:10)){
	...
	test_dat$PO = unlist(lapply(test_dat$Reciprocal, function(x) ifelse(length(grep("a", x)) > 0, 0.5, -0.5)))
   	test_dat$pup_gene = factor(test_dat$pup_gene)
		...
		try(fit_list[[g]] <- vglm(cbind(mat_tot, pat_tot) ~ PO + Diet, betabinomial,
	...
	saveRDS(fit_list, paste0(dir, "/ase_vgam/ase_PODIET_rix",r,"_1may2021.rds"))  
#}
